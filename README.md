...
Copy `.env.example` to `.env` and fill the required secrets. Login emails try Unosend first (`UNOSEND_API_KEY` + verified `UNOSEND_FROM_EMAIL`), then fall back to SendGrid (`SENDGRID_API_KEY` + `SENDGRID_FROM_EMAIL`). If both are unavailable, the app logs the unsent message and fails the send.
