...
Copy `.env.example` to `.env` and fill the required secrets. `NODE_ENV` is read from `.env`; `development` creates sandbox/test API keys, `production` creates live API keys. `TRANSACTIONAL_EMAIL_MOCK_OVERRIDE=false` sends login emails via SendGrid first (`SENDGRID_API_KEY` + `SENDGRID_FROM_EMAIL`), then falls back to Unosend (`UNOSEND_API_KEY` + verified `UNOSEND_FROM_EMAIL`). Set `TRANSACTIONAL_EMAIL_MOCK_OVERRIDE=true` to print emails to logs instead. If real providers are unavailable, the app logs the unsent message and fails the send.
